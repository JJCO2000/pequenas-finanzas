...
export function KingGreedyGame({ session, onFinish }: GameComponentProps) {
  ...
  useEffect(() => {
    ...
    const loop = Animated.loop(Animated.sequence([
      Animated.timing(pointerPulse, { toValue: 1, duration: 145, useNativeDriver: true }),
      Animated.timing(pointerPulse, { toValue: 0, duration: 155, useNativeDriver: true }),
    ]));
    loop.start();
    return () => loop.stop();
  ...
  const startSpin = () => {
    ...
    const fullTurns = 5 + (spinsRef.current % 2);
    ...
    <ImageBackground source={ACTIVE_THEME.world.lesson} resizeMode="cover" style={styles.root}>
      ...
      <View style={styles.board}>
        <View style={styles.wheelPane}>
          ...
          <View style={styles.wheelFrame}>
            ...
            <View style={styles.center}>
              ...
              <Text style={styles.centerMeta}>{spinning ? 'La rueda decidirá' : active === null ? 'Gira y decide después' : 'Ahora decide si aseguras'}</Text>
            </View>
              ...
              style={[
                styles.pointer,
                {
                  transform: [
                    ...
                    { scale: pointerPulse.interpolate({ inputRange: [0, 1], outputRange: [1, 1.10] }) },
                  ],
                },
              ]}
            >
          ...
          <Text style={styles.wheelInstruction}>{spinning ? '🎡 La rueda está girando…' : '🎯 El puntero de arriba marca el resultado'}</Text>
        </View>
        ...
        <View style={styles.riskPane}>
          ...
          <View style={styles.feedbackSlot}>{feedback ? <FeedbackPill text={feedback.text} good={feedback.good} /> : <Text style={styles.tip}>La ruleta sí gira: el riesgo aparece primero y tu decisión viene después.</Text>}</View>
        </View>
      </View>
    </ImageBackground>
  );
...
const styles = StyleSheet.create({
  ...
  wheelShadow: { position: 'absolute', width: 222, height: 42, borderRadius: 111, backgroundColor: 'rgba(20,20,14,0.34)', transform: [{ translateY: 96 }, { scaleX: 0.9 }] },
  wheelFrame: { width: 250, height: 236, alignItems: 'center', justifyContent: 'center', position: 'relative' },
  wheel: { width: 232, height: 220, borderRadius: 110, backgroundColor: '#5B3E2A', borderWidth: 8, borderColor: '#D6A928', position: 'absolute', ...shadows.card },
  wheelInnerRing: { position: 'absolute', left: 18, right: 18, top: 18, bottom: 18, borderRadius: 88, borderWidth: 4, borderColor: 'rgba(255,232,142,0.55)', backgroundColor: 'rgba(15,80,53,0.38)' },
  ...
  spaceActive: { backgroundColor: colors.orange, borderColor: colors.white, zIndex: 5 },
  ...
  spaceText: { color: colors.white, fontSize: 8.2, lineHeight: 10, fontWeight: '900', textAlign: 'center' },
  center: { position: 'absolute', width: 92, height: 84, borderRadius: 46, backgroundColor: colors.gold, borderWidth: 4, borderColor: '#FFF4C5', alignItems: 'center', justifyContent: 'center', padding: 7, overflow: 'hidden', zIndex: 8, ...shadows.soft },
  ...
  centerTitle: { color: colors.forestDark, fontSize: 11.5, fontWeight: '900' },
  centerMeta: { color: colors.ink, fontSize: 6.5, lineHeight: 9, fontWeight: '800', textAlign: 'center' },
  pointer: { position: 'absolute', top: -5, width: 34, height: 36, borderTopLeftRadius: 16, borderTopRightRadius: 16, borderBottomLeftRadius: 8, borderBottomRightRadius: 8, backgroundColor: '#F26A2E', borderWidth: 3, borderColor: '#FFF0C0', alignItems: 'center', justifyContent: 'center', zIndex: 12, ...shadows.card },
  pointerGem: { width: 10, height: 10, borderRadius: 5, backgroundColor: '#FFE269', borderWidth: 2, borderColor: '#9A6913' },
  pointerTip: { position: 'absolute', bottom: -13, width: 0, height: 0, borderLeftWidth: 9, borderRightWidth: 9, borderTopWidth: 15, borderLeftColor: 'transparent', borderRightColor: 'transparent', borderTopColor: '#F26A2E' },
  wheelInstruction: { color: colors.cream, fontSize: 7.5, fontWeight: '900', backgroundColor: 'rgba(5,67,45,0.86)', borderRadius: radii.pill, paddingHorizontal: 10, paddingVertical: 4, marginTop: -2 },
