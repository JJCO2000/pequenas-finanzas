...
export type HomeLayout = {
  ...
  destinationPanelTopOffset: number;
  destinationGap: number;
  destinationRowGap: number;
...
export function resolveHomeLayout(width: number, height: number, insetLeft = 0, insetRight = 0, insetTop = 0, insetBottom = 0): HomeLayout {
  ...
  const destinationPanelPadding = mode === 'compact' ? 8 : mode === 'expanded' ? 20 : 18;
  const destinationPanelBorderWidth = HOME_DESTINATION_PANEL_BORDER_WIDTH;
  const destinationPanelTopOffset = mode === 'compact' ? 0 : mode === 'expanded' ? 4 : 8;
  ...
  const destinationCardHeight = mode === 'compact' ? 64 : mode === 'expanded' ? 187 : 177;
