...
assert.match(destinationsSource, /background: require\('[^']*tienda\/background\.webp'\)/, 'Tienda background must remain explicit');
assert.match(destinationsSource, /eggNest: require\('[^']*tienda\/egg-nest\.webp'\)/, 'Tienda egg and nest must be one named canonical foreground layer');
